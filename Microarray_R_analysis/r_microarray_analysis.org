...
4. Go to ~Code~ -> ~Run region~ -> ~Run All~. You can do the same by pressing ~ctl+Shift+Enter~.
5. A folder with your chosen name will appear on the same directory where you have the script with the results of your analysis.

#+LaTeX: \pagebreak

* Script: Modifiable Parts
:PROPERTIES:
:header-args:R: :session R1 :tangle microarray_analysis.R :comments none :results none :colnames yes
:END:
...
:PROPERTIES:
:header-args:R: :session R1 :tangle microarray_analysis.R :comments org :results none :exports none
:END:
